...
class Bot(BaseBot):
    async def on_start(self, session_metadata: SessionMetadata) -> None:
        print("working")
        await self.highrise.walk_to(Position(15.5 , 2.25 , 25.5 , "FrontRight"))
    async def on_user_join(self, user: User, position: Position | AnchorPosition) -> None:
        print(f"{user.username} entrou na sala")   
        await self.highrise.chat(f"✨️Welcome {user.username} Use: !emote list or 1-97 For Dances & Emotes")

        await self.highrise.chat(f"✨️Use: /help For More Informations.")
    ...
    async def on_chat(self, user: User, message: str) -> None:
        ...
        if message.lower().startswith("!tipall ") and user.username == "____Rayen____" or user.username == "eva__63" or user.username == "superhotgirll":
              parts = message.split(" ")
              if len(parts) != 2:
              ...
              for room_user, pos in room_users.content:
                  ...
                  for bar in tip:
                      ...
                      await self.highrise.chat(f"💰Tipped {room_user.username} {amount} gold!")

        if message.lower().startswith("!tipme ") and user.username == "____Rayen____" or user.username == "eva__63" or user.username == "superhotgirll":
                try:
        ...
        if        message.startswith("/tele") or              message.startswith("/tp") or              message.startswith("/fly") or     message.startswith("!tele") or      message.startswith("!tp") or     message.startswith("!fly"):
          if user.username == "____Rayen____" or user.username == "eva__63" or user.username == "superhotgirll":            await self.teleporter(message)
        ...
        if              message.startswith("Fairyfloat All") or                              message.startswith("/emote all fairyfloat") or       message.startswith("!emote all fairyfloat"):
          if user.username == "____Rayen____" or user.username == "eva__63" or user.username == "superhotgirll":
